...
public class IOCommands implements CommandMarker
{
	 ...
	 */
	static final         String IMPORT_FILE_PATH_OPTION = "FilePath";
	/**
...
//		}
//		return IOCommands.FAILED_EXPORT;
		return IOCommands.SUCCESSFUL_EXPORT;
	}
	...
	private static void logSevereError(final String message, final Exception exception)
	{
		IOCommands.LOGGER.log(Level.SEVERE, message);
		IOCommands.LOGGER.log(Level.SEVERE, exception.getMessage());
		IOCommands.LOGGER.log(Level.SEVERE, exception.toString(), exception.fillInStackTrace());
	}
	...
	public String importPGN(
			@CliOption(key = {IOCommands.IMPORT_FILE_PATH_OPTION}, mandatory = true, help = "Path to the PGN file to be imported")
						   ...
						   ) throws IOException
	{
		...
		if(failureDetails == null)
		{
			return IOCommands.SUCCESSFUL_IMPORT
				   + IOCommands.SPACE
				   + this.commandContext.getChessIO().getGamesCount()
				   + IOCommands.SPACE
	...
	private void handleAndThrowIOPGNError(final IOException ioException) throws IOException
	{
		...
		IOCommands.logSevereError(failureDetails, ioException);
		throw ioException;
	}
}
